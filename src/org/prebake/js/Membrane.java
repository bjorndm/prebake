...
final class Membrane {
  ...
  private final class NativeArrayWrapper extends AbstractList<Object> {
    private final NativeArray arr;

    NativeArrayWrapper(NativeArray arr) { this.arr = arr; }
  ...
  private List<?> makeListWrapper(final NativeArray arr) {
    return wrap(arr, new NativeArrayWrapper(arr));
  }

  private final class BaseFunctionWrapper implements MembranableFunction {
    private final BaseFunction fn;

    BaseFunctionWrapper(BaseFunction fn) { this.fn = fn; }
    ...
    public int getArity() { return fn.getArity(); }
    public Documentation getHelp() { return null; }
    public String getName() { return fn.getFunctionName(); }
  }

  private Function<Object[], Object> makeFnWrapper(BaseFunction fn) {
    return wrap(fn, new BaseFunctionWrapper(fn));
  }

  private final class ScriptableObjectWrapper
      extends AbstractMap<String, Object> {
    private final ScriptableObject obj;

    ScriptableObjectWrapper(ScriptableObject obj) { this.obj = obj; }
  ...
  private Map<String, Object> makeMapWrapper(final ScriptableObject obj) {
    return wrap(obj, new ScriptableObjectWrapper(obj));
  }
  ...
  private final class JavaArrayWrapper implements Scriptable {
    private final Object array;
    private final int n;
    private final Class<?> cType;

    JavaArrayWrapper(Object array) {
      this.array = array;
      this.n = Array.getLength(array);
      this.cType = array.getClass().getComponentType();
    }
  ...
  private Scriptable makeArrayWrapper(Object array) {
    return wrap(array, new JavaArrayWrapper(array));
  }

  private final class ListWrapper implements Scriptable {
    private final List<?> list;
    @Nullable private final List<Object> writeFacet;
    ListWrapper(List<?> list, @Nullable List<Object> writeFacet) {
      this.list = list;
      this.writeFacet = writeFacet;
    }
  ...
  private Scriptable makeListWrapper(
      List<?> list, @Nullable List<Object> writeFacet) {
    return wrap(list, new ListWrapper(list, writeFacet));
  }

  private final class MapWrapper implements Scriptable {
    private final Map<?, ?> map;
    @Nullable private final Map<String, Object> writeFacet;

    MapWrapper(Map<?, ?> map, @Nullable Map<String, Object> writeFacet) {
      this.map = map;
      this.writeFacet = writeFacet;
    }
  ...
  private Scriptable makeObjWrapper(
    ...
    return wrap(map, new MapWrapper(map, writeFacet));
  }
